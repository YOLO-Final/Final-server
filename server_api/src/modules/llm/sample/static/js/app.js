const chatBox = document.getElementById('chat-box');
    ...
    const WEB_SEARCH_ENABLED_KEY = 'bao_web_search_enabled';
    ...
    function setWebSearchEnabled(enabled) {
        const webSearchEnabled = enabled !== false;
        if (webSearchToggle) {
            webSearchToggle.checked = webSearchEnabled;
        }
        if (quickWebSearchToggle) {
            quickWebSearchToggle.checked = webSearchEnabled;
        }
        localStorage.setItem(WEB_SEARCH_ENABLED_KEY, webSearchEnabled ? 'true' : 'false');
    }
    ...
    function deleteSingleHistory(topicId) {
        ...
        const ok = confirm(`"${truncateTopic(target.topic, 60)}" 기록을 삭제하시겠습니까?`);
        if (!ok) return;
    ...
    async function resetServerMemorySilently() {
        try {
            ...
            fd.append('session_id', activeTopicId || 'default');
            await fetch(`${LLM_API_BASE}/memory/reset`, { method: 'POST', body: fd });
        } catch (e) {
    ...
    function attachUserMessageActions(messageDiv, originalText) {
        ...
        editBtn.innerText = 'Edit and resend';
        editBtn.title = 'Load this message into input, edit it, and send again.';
    ...
    async function generateImageFromPrompt(promptText) {
        ...
        const res = await fetch(`${LLM_API_BASE}/image`, { method: 'POST', body: fd });
    ...
    async function speakViaServer(text) {
        ...
        const res = await fetch(`${LLM_API_BASE}/tts`, { method: 'POST', body: fd });
    ...
    async function speak(text, token = null) {
        ...
        if (ttsMode === 'server') {
            try {
                ...
                console.warn('Server TTS failed, fallback to browser TTS:', e);
                ttsMode = 'browser';
    ...
    async function sendMessage() {
        ...
        if (isImageGenerationRequest(text)) {
            ...
            try {
                ...
                addMsg('ai', '요청하신 이미지를 생성했습니다.');
                addImageMsg(imageSrc, text);
                // Data URLs can be very large for localStorage, so keep only remote URLs in history.
        ...
        fd.append('disable_auto_web', webSearchToggle.checked ? 'false' : 'true');
        fd.append('session_id', activeTopicId || 'default');
        const controller = new AbortController();
        activeChatController = controller;

        try {
            const res = await fetch(`${LLM_API_BASE}/chat`, { method: 'POST', body: fd, signal: controller.signal });
    ...
    async function uploadFile() {
        ...
        try {
            const res = await fetch(`${LLM_API_BASE}/upload`, { method: 'POST', body: fd });
            const data = await res.json();
            if (data.error) {
                throw new Error(data.error);
            }
            const details = data.indexed_chunks !== undefined
                ? `${data.message || 'Upload complete'}\nFiles: ${(data.files || []).length}\nIndexed chunks: ${data.indexed_chunks}`
                : (data.message || 'Upload complete');
            alert(details);
            hideWelcome();
            fileInput.value = '';
    ...
    async function resetMemory() {
        ...
        try {
            const fd = new FormData();
            fd.append('session_id', activeTopicId || 'default');
            const res = await fetch(`${LLM_API_BASE}/memory/reset`, { method: 'POST', body: fd });
            const data = await res.json();
        ...
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                if (audioBlob.size < 1500) {
                    addMsg('ai', '음성이 너무 짧아 인식하지 못했습니다. 1~2초 이상 말한 뒤 다시 시도해 주세요.');
                    if (currentMicStream) {
                        currentMicStream.getTracks().forEach(track => track.stop());
                        currentMicStream = null;
                    }
                ...
                try {
                    const res = await fetch(`${LLM_API_BASE}/stt`, { method: 'POST', body: fd });
                    const data = await res.json();
                    if (data.error) {
    ...
    function toggleSidebar() {
        ...
        if (railMenuBtn) {
            ...
            railMenuBtn.setAttribute('aria-label', isOpen ? 'Close menu' : 'Open menu');
        }
    ...
    function closeSidebar() {
        ...
        if (railMenuBtn) {
            ...
            railMenuBtn.setAttribute('aria-label', 'Open menu');
        }
    ...
    function applyQuickSettingsToMain() {
        ...
        if (webSearchToggle && quickWebSearchToggle) {
            setWebSearchEnabled(quickWebSearchToggle.checked);
        }
    ...
    clearHistoryBtn.addEventListener('click', () => {
        const ok = confirm('전체 기록을 삭제하시겠습니까?');
        if (!ok) return;
        chatTopics = [];
        activeTopicId = null;
        saveTopicHistory();
    ...
    if (webSearchToggle) {
        webSearchToggle.addEventListener('change', () => setWebSearchEnabled(webSearchToggle.checked));
    }
    if (ttsToggle) {
    ...
    const savedWebSearchEnabled = localStorage.getItem(WEB_SEARCH_ENABLED_KEY);
