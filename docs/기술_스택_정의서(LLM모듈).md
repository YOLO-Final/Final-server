...
> 런타임 전환: API 요청의 `provider` 파라미터로 `openai` / `gemini` / `vllm` / `vllm_fast` 중 선택

> Gemini는 SDK(`google-generativeai`) 외에 `llm_simple.py`에서 `urllib.request`로 REST 직접 호출도 병행
...
| langchain-google-genai | 2.0.0 | Google GenAI + LangChain 연동 |
